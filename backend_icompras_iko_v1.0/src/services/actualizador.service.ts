...
function descargarZip(url: string, destino: string, redireccionesRestantes = 5): Promise<void> {
    return new Promise((resolve, reject) => {
        ...
        mod.get(url, { headers: { 'User-Agent': 'icompras-updater' } }, (res) => {
...
export async function ejecutarActualizacion(): Promise<ResultadoActualizacion> {
    ...
    const ZIP_URL = cfg.githubZipUrl || '';

    if (!ZIP_URL) {
        return { success: false, descarga: '', archivosCopiados: 0, reiniciando: false,
                 mensaje: 'URL de GitHub no configurada. Configurala en la página de Configuración.', log };
    }
    ...
    try {
        ...
        limpiar();
        return { success: false, descarga: ZIP_URL, archivosCopiados: 0, reiniciando: false,
                 mensaje: `Falló la descarga: ${err.message}`, log };
    }

    // 2. Extraer ZIP
    let origenReal: string;
    try {
        paso('Extrayendo ZIP...');
        ...
        origenReal = path.join(EXTRACT_TMP, subDir);
    } catch (err: any) {
        paso(`ERROR al extraer ZIP: ${err.message}`);
        limpiar();
        return { success: false, descarga: ZIP_URL, archivosCopiados: 0, reiniciando: false,
                 mensaje: `Falló la extracción: ${err.message}`, log };
    }
    ...
    try {
        ...
        limpiar();
        return { success: false, descarga: ZIP_URL, archivosCopiados, reiniciando: false,
                 mensaje: `Falló la copia: ${err.message}`, log };
    }

    limpiar();
    paso('Temporales eliminados.');

    // 4. Reiniciar con NSSM
    ...
    if (!servicioBackend && !servicioFrontend) {
        ...
        return { success: true, descarga: ZIP_URL, archivosCopiados, reiniciando: false,
                 mensaje: `${archivosCopiados} archivos actualizados. Reinicia el backend manualmente.`, log };
    }
    ...
    return { success: true, descarga: ZIP_URL, archivosCopiados, reiniciando: true,
             mensaje: `${archivosCopiados} archivos actualizados. Reiniciando servicios NSSM en ~3 segundos.`, log };
}
