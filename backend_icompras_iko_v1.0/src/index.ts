...
import triangulacionRouter  from "./routers/triangulacion.router";
import pedidosCtrlRouter    from "./routers/pedidosCtrl.router";
...
app.use('/triangulacion',  triangulacionRouter);
app.use('/pedidos-ctrl',   pedidosCtrlRouter);

app.listen(port, async () => {
    console.log(`Servidor en http://localhost:${port}`);
    await PromocionesService.initTablas();
    await EcommerceService.initTablas();
    await TriangulacionService.initTablas();
